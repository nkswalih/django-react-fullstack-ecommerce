...
const colorMap = {
  ...
  'space gray': 'bg-zinc-600', 'midnight': 'bg-slate-900', 'titanium': 'bg-stone-400',
};
...
const AccessoriesPage = () => {
  ...
  const all = products.filter(p => p.category === 'Accessory');
  const apple = all.filter(p => p.brand === 'Apple');
  ...
  if (loading) return (
    <div className="min-h-screen bg-gradient-to-br from-[#d9e8f5] via-[#e2ebf4] to-[#f4f7fa] flex items-center justify-center">
      <div className="flex flex-col items-center gap-3">
        ...
        <p className="text-gray-500 text-sm font-medium">Loading accessories...</p>
      </div>
    </div>
  );
